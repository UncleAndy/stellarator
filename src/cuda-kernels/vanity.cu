...
struct Config {
	...
	char            suffix[64];
	size_t          suffix_count;
	size_t          stop_at_count;
};

/* -- Prototypes, Because C++ ----------------------------------------------- */

extern "C" void vanity_setup(Config& vanity);
extern "C" void vanity_run(Config& vanity);
void __global__ vanity_init(unsigned long long int* seed, curandState* state);
void __global__ vanity_scan(curandState* state, int* keys_found, int* gpu, int* execution_count, Config vanity);
bool __device__ b58enc(char* b58, size_t* b58sz, uint8_t* data, size_t binsz);

/* -- Entry Point ----------------------------------------------------------- */

/*
int main_disabled(int argc, char const* argv[]) {
...
void vanity_run(Config &vanity) {
	...
	for (int i = 0; i < MAX_ITERATIONS; ++i) {
		...
		for (int g = 0; g < gpuCount; ++g) {
			...
			vanity_scan<<<maxActiveBlocks, blockSize>>>(vanity.states[g], dev_keys_found[g], dev_g[g], dev_executions_this_gpu[g], vanity);
		...
		printf("%s Iteration %d Attempts: %llu in %f at %fcps - Total Attempts %llu - keys found %d\n",
			...
			keys_found_total
                ...
                if ( keys_found_total >= (int)vanity.stop_at_count ) {
...
static __device__ uint16_t crc16_xmodem(const uint8_t *data, size_t len) {
    uint16_t crc = 0x0000;
...
static __device__ void encode_base32(char *out, const uint8_t *data, size_t len) {
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        bits += 8;
        while (bits >= 5) {
            out[out_idx++] = alphabet[(buffer >> (bits - 5)) & 31];
            bits -= 5;
        }
    }
    if (bits > 0) {
        out[out_idx++] = alphabet[(buffer << (5 - bits)) & 31];
    }
    out[out_idx] = '\0';
}

static __device__ void stellar_encode_seed(char *out, const uint8_t *seed) {
    uint8_t payload[35];
    payload[0] = 18 << 3; // Version byte for 'S' (Secret seed)
    for (int i = 0; i < 32; ++i) payload[i + 1] = seed[i];
    uint16_t crc = crc16_xmodem(payload, 33);
    payload[33] = (uint8_t)(crc & 0xFF);
    payload[34] = (uint8_t)((crc >> 8) & 0xFF);
    encode_base32(out, payload, 35);
}

static __device__ void stellar_encode_public_key(char *out, const uint8_t *public_key) {
    uint8_t payload[35];
    payload[0] = 6 << 3; // Version byte for 'G' (Account ID)
    for (int i = 0; i < 32; ++i) payload[i + 1] = public_key[i];
    uint16_t crc = crc16_xmodem(payload, 33);
    payload[33] = (uint8_t)(crc & 0xFF);
    payload[34] = (uint8_t)((crc >> 8) & 0xFF);
    encode_base32(out, payload, 35);
...
void __global__ vanity_scan(curandState* state, int* keys_found, int* gpu, int* exec_count, Config vanity) {
	int id = threadIdx.x + (blockIdx.x * blockDim.x);
	...
	for (int attempts = 0; attempts < ATTEMPTS_PER_EXECUTION; ++attempts) {
		...
		stellar_encode_public_key(key, publick);

		// Code Until here runs fast enough for Stellar.

		// Check Prefix and Suffix
                bool match = true;
                if (vanity.prefix_count > 0) {
                    for (int j = 0; j < vanity.prefix_count; ++j) {
                        if (vanity.prefix[j] != '?' && vanity.prefix[j] != key[j]) {
                            match = false;
                            break;
                        }
                    }
                ...
                if (match && vanity.suffix_count > 0) {
                    int key_len = 56;
                    int start_pos = key_len - (int)vanity.suffix_count;
                    for (int j = 0; j < vanity.suffix_count; ++j) {
                        if (vanity.suffix[j] != '?' && vanity.suffix[j] != key[start_pos + j]) {
                            match = false;
                            break;
                        }
                    }
                ...
                if (match) {
                        ...
                        char stellar_seed[57];
                        stellar_encode_seed(stellar_seed, seed);
                        printf("GPU %d MATCH %s - %s\n", *gpu, key, stellar_seed);
